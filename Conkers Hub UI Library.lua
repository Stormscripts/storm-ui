...
-- ThemeAllRegistry: callbacks para ThemeAll (afeta absolutamente TUDO)
local ThemeAllRegistry = {}

local function RegisterThemeAll(inst, prop, key)
    table.insert(ThemeAllRegistry, {inst = inst, prop = prop, key = key})
end

-- ApplyThemeAllRegistry: aplica ThemeAllRegistry (global absoluto)
-- Suporta callbacks da mesma forma que ApplyThemeAll
local function ApplyThemeAllRegistry()
    for _, entry in ipairs(ThemeAllRegistry) do
        if entry.callback then
            pcall(entry.callback)
        elseif entry.inst and entry.inst.Parent then
            pcall(function() entry.inst[entry.prop] = Theme[entry.key] end)
...
-- Registra um callback no ThemeAllRegistry (para listas dinâmicas globais)
local function RegisterThemeAllCallback(fn)
    table.insert(ThemeAllRegistry, {callback = fn})
end

-- ================================================================
-- SetThemeComp: afeta apenas componentes no ThemeRegistry (parcial)
...
-- ================================================================
-- ThemeAll: afeta absolutamente TUDO registrado (ThemeAllRegistry + ThemeRegistry)
-- ================================================================
function ConkersHub:ThemeAll(name)
    local p = Themes[name]
    if not p then
        warn("ConkersHub: Tema '" .. tostring(name) .. "' inválido para ThemeAll.")
        return
    end
    for k, v in pairs(p) do Theme[k] = v end
    -- Aplica ThemeRegistry (parcial) + ThemeAllRegistry (global)
    ApplyThemeAll()
    ApplyThemeAllRegistry()
end
...
function ConkersHub:CreateWindow(config)
    ...
    local function MakeComponents(container)
        ...
        -- ThemeAllSelector: mesmo visual do ThemeCompSelector mas chama ThemeAll
        function obj:AddThemeAllSelector(cfg)
            cfg = cfg or {}
            local name = cfg.Name or "Theme All"
            local default = cfg.Default or "Purple"
            local callback = cfg.Callback or function() end
            local themeNames = {"Purple", "Blue", "Green", "Red", "Orange", "Pink", "Cyan", "White", "Dark"}
            local themeAccents = {
            ...
            RegisterThemeAll(TitleLbl, "TextColor3", "TextPrimary")
            ...
            local function refreshDots()
                for tname, dot in pairs(dots) do
                    ...
                    local stroke = dot:FindFirstChildOfClass("UIStroke")
                    if stroke then stroke.Thickness = isActive and 2 or 0 end
                end
            end
            ...
            for _, tname in ipairs(themeNames) do
                ...
                Corner(Dot, 9)
                Stroke(Dot, Color3.fromRGB(255, 255, 255), 0)
                dots[tname] = Dot
                ...
                DotBtn.MouseButton1Click:Connect(function()
                    currentTheme = tname
                    ConkersHub:ThemeAll(tname)
                    refreshDots()
                    callback(tname)
                end)
                DotBtn.MouseEnter:Connect(function()
                    if tname ~= currentTheme then Tween(Dot, 0.12, {Size = UDim2.new(0, 21, 0, 21)}) end
                end)
                DotBtn.MouseLeave:Connect(function()
                    if tname ~= currentTheme then Tween(Dot, 0.12, {Size = UDim2.new(0, 18, 0, 18)}) end
                end)
            end

            if default and Themes[default] then
                ConkersHub:ThemeAll(default)
            end
            refreshDots()

            local TAS = {}
            function TAS:Set(tname)
                if Themes[tname] then
                    currentTheme = tname
                    ConkersHub:ThemeAll(tname)
                    refreshDots()
            ...
            function TAS:Get() return currentTheme end
            return TAS
        end
    ...
    -- SetTransparency: invisibilidade real dos backgrounds da interface.
    -- alpha=0 → totalmente visível (original)
    -- alpha=1 → totalmente invisível
    -- NÃO afeta: textos (TextLabel/TextBox), FloatBtn, DialogOverlay e seus filhos.
    -- Afeta: Frame, ScrollingFrame, ImageLabel, ImageButton, TextButton (fundo).
    -- Fórmula: result = 1 - (1 - baseTransparency) * (1 - alpha)
    --   → Preserva elementos já transparentes (base > 0).
    --   → alpha=0 reproduz exatamente o estado original.
    --   → alpha=1 torna tudo invisível independente do base.
    ...
    local function isFloatBtn(inst)
        -- FloatBtn fica no ScreenGui diretamente, não dentro de Main
        return inst.Name == "ConkersHubFloat"
    end
    ...
    local function buildSnapshot()
        ...
        local function collect(inst)
            for _, child in ipairs(inst:GetChildren()) do
                -- Ignora Dialog e filhos
                if isDialogDescendant(child) then
                    -- skip
                -- Ignora textos (nunca afetar)
                elseif child:IsA("TextLabel") or child:IsA("TextBox") then
                    collect(child)
                -- Afeta backgrounds de frames e botões
        ...
        -- Coleta apenas dentro de Main (FloatBtn não é filho de Main)
        collect(Main)
    end
    ...
    function Window:SetTransparency(alpha)
        ...
        for inst, baseT in pairs(transparencySnapshot) do
            if inst and inst.Parent then
                -- Invisibilidade real: interpola base → 1 conforme alpha
                local result = 1 - (1 - baseT) * (1 - alpha)
                pcall(function() inst.BackgroundTransparency = result end)
            end
        end
    end

    -- Reconstrói snapshot (chamar após trocar tema ou adicionar novos elementos)
    ...
    function Window:AddTab(tabConfig)
        ...
        Tab.ThemeAllSelector = Tab.AddThemeAllSelector
        ...
        function Tab:AddSection(sectionConfig)
            ...
            sec.ThemeAllSelector = sec.AddThemeAllSelector
