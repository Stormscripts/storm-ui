--[[
    ...
    ╠══════════════════════════════════════════════╣
    ║  Correções v1.0:                             ║
    ║  1. Cantos arredondados reais                ║
    ║     ClipsDescendants = false + UICorner em   ║
    ║     cada frame filho nos cantos corretos.    ║
    ║                                              ║
    ║  2+3. Drag sem teleporte                     ║
    ║     AbsolutePosition armazenado UMA VEZ no   ║
    ║     início. novaPos = absInicio + delta.     ║
    ║                                              ║
    ║  4. FloatBtn sem sombra                      ║
    ║  5. Title descido levemente                  ║
    ╚══════════════════════════════════════════════╝
...
-- ─── [Fix 2+3] Drag sem teleporte ─────────────────────────────────────────────
-- Armazena frame.AbsolutePosition UMA VEZ ao iniciar o drag.
-- Cada update calcula: novaPos = absInicio + delta acumulado.
-- Sem conversão de escala → sem teleporte.
local function MakeDraggable(handle, frame)
    local dragging      = false
    local dragStartInput = nil  -- posição do input no início
    local frameStartAbs  = nil  -- AbsolutePosition do frame no início
    ...
    handle.InputBegan:Connect(function(input)
        ...
        or input.UserInputType == Enum.UserInputType.Touch then
            ...
            frameStartAbs  = frame.AbsolutePosition  -- lido UMA VEZ, aqui
        end
    end)
    ...
    UserInputService.InputChanged:Connect(function(input)
        ...
        or input.UserInputType == Enum.UserInputType.Touch then
            ...
            frame.Position = UDim2.new(
                0, math.clamp(frameStartAbs.X + delta.X, -frame.AbsoluteSize.X + 40, vp.X - 40),
                0, math.clamp(frameStartAbs.Y + delta.Y, 0, vp.Y - 40)
            )
        end
    end)
...
function ConkersHub:CreateWindow(config)
    ...
    -- Preenche cantos inferiores do TopBar (faz ficarem retos)
    Create("Frame", {
    ...
    function Window:AddFloatButton(cfg)
        ...
        local DRAG_THRESHOLD  = 8  -- pixels para considerar arrasto
        ...
        FloatBtn.InputBegan:Connect(function(input)
            ...
            or input.UserInputType == Enum.UserInputType.Touch then
                fDragging       = true
                fWasDragged     = false
                fDragStartInput = input.Position
                fFrameStartAbs  = FloatBtn.AbsolutePosition  -- lido UMA VEZ
            end
        end)
        ...
        UserInputService.InputChanged:Connect(function(input)
            ...
            if input.UserInputType == Enum.UserInputType.MouseMovement
            or input.UserInputType == Enum.UserInputType.Touch then
                local delta = input.Position - fDragStartInput
                if delta.Magnitude > DRAG_THRESHOLD then
                    fWasDragged = true
                end
                local vp   = workspace.CurrentCamera.ViewportSize
                local newX = math.clamp(fFrameStartAbs.X + delta.X, 0, vp.X - FloatBtn.AbsoluteSize.X)
                local newY = math.clamp(fFrameStartAbs.Y + delta.Y, 0, vp.Y - FloatBtn.AbsoluteSize.Y)
